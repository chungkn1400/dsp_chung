...
'
Dim Shared As Double auxvar,auxvar2,auxvar3,auxvar4 
...
While quit=0 And guitestkey(vk_escape)=0
   ...
   If Timer>timemsg+0.2 Then
    timemsg=Timer 
    msg="freeout="+Str(freeout)+"/"+Str(mynsamples)+"/"+Str(mypsamples[0])+crlf
    If play=1 Then msg+="out="+Str(okout)
    msg+="/"+errtext
...
play=1
While quit=0 And guitestkey(vk_escape)=0
	If Timer>timepeek+0.1 Then
		timepeek=Timer
		mypeek=0
	EndIf
	guiscan
	Sleep 20
	avgpeek+=(mypeek-avgpeek)*0.5
	If avgpeek>28000 Then Exit While
	If Timer>timetest+1 Then Exit While
Wend
gain=gain0
play=play0
If avgpeek<28000 Then Exit Sub 
'guinotice "testloop"
timetest=Timer 
avgpeek=0
gain=0
