' gui_chung a program by NGUYEN.Chung (freeware 2010-2018)
...
Using GDIPLUS
Function ImageLoad(Filename as WString) as hbitmap
'bmp gif jpg ico png tif
If FileExists(filename)=0 Then Return 0  
   ...
   'Dim As HBITMAP hBitmap = LoadImage( 0, "fblogo.bmp", IMAGE_BITMAP , 0, 0, LR_LOADFROMFILE )

   Dim As hbitmap hBitmap
   'Dim As gpbitmap Ptr gbitmap
   Dim As Any Ptr gbitmap
   GdipCreateBitmapFromfile(WStr(Filename), @gBitmap)
   'GetObject( @hBitmap, sizeof( gbitmap ), gbitmap )
   GdipCreateHBITMAPFromBitmap(gbitmap, @hbitmap, RGBA(0,0,0,0)) 
   'guinotice "gbitmap="+str(gbitmap)+" hbitmap="+str(hbitmap)
   Dim As uint w,h
   'Dim As gpimage Ptr gimage
   'GdiploadimageFromfile(WStr(Filename), @gimage)
   'GdipGetImageWidth(gimage, @w)
   'GdipGetImageHeight(gimage, @h)
   GdipGetImageWidth(gbitmap, @w)
   GdipGetImageHeight(gbitmap, @h)
   ...
   Return hbitmap
End Function
Function hbmpjpg(ByVal hwin As hwnd,Byval filename As ZString ptr) As hbitmap
Return ImageLoad(WStr(*filename)) 
End Function 
/'
...
Function guiloadimage(ByRef filename As String) As hbitmap
        'Supported types; bmp,jpg,ico,gif,png,tif
If FileExists(filename)=0 Then Return 0 
...
Sub guiscan
Dim guimsg As MSG
Dim As Integer guistatus,i,j 
If guiend=0 Then 
	guistatus=1:i=0
	While guistatus<>0 And i<1000
		i+=1
		'If GetQueueStatus(QS_ALLINPUT) Then 'Or QS_ALLEVENTS) Then  
		If peekmessage(@guimsg,NULL,0,0,PM_NOREMOVE)<>0 Then 
   		  guistatus=GetMessage(@guimsg,NULL,0,0)
		  '   If guimsg.message=WM_QUIT Then status=0
		     If guistatus<>0 And guistatus<>-1 Then 
			   TranslateMessage(@guimsg)
	   	   DispatchMessage(@guimsg)
		     EndIf
	        'If guistatus=-1 Then guinotice("getmsg error guistatus=-1","guiscan")
		Else
			i=10000
		EndIf
	Wend
	If guistatus=0 Then guiend=1
...
Sub guiwait
Dim guimsg As MSG
Dim As Integer guistatus,i
guiscan
guistatus=GetMessage(@guimsg,NULL,0,0)
If guimsg.message=WM_QUIT Then guistatus=0
If guistatus<>0 And guistatus<>-1 Then 
	TranslateMessage(@guimsg)
	DispatchMessage(@guimsg)
EndIf	
...
guiaddcontrol(controlname,"scroll",controlsub,_ 
	        controlx,controly,controldx,controldy,userflags)
End Sub
Sub getscrollbarpos(Byref controlname As String,ByRef i As Integer)
   i=getscrollpos(getguih(controlname),sb_ctl)
End Sub 
Sub setscrollbarpos(Byref controlname As String,Byval i As Integer)
	SetScrollPos(getguih(controlname),sb_ctl,i,TRUE)
End Sub 
Sub getscrollbarrange(Byref controlname As String,Byref imin As Integer,Byref imax As integer)
   Var xmin=0,xmax=10
   getscrollrange(getguih(controlname),sb_ctl,@xmin,@xmax)
   imin=xmin:imax=xmax
End Sub 
Sub setscrollbarrange(Byref controlname As String,Byval imin As Integer,ByVal imax As integer)
   SetScrollRange(getguih(controlname),sb_ctl,imin,imax,TRUE)
End Sub 
...
Function guiBload( ByVal bmpbuffer As Any Ptr=0, _   
	                ByRef filename As String,ByVal maxcolor As uint=255,_  
	                     ...
	                     'bmp jpg gif ico png tif-> freebasic screen or bmp buffer
Dim As hbitmap hbmp,hbmp2
Dim As hdc bmpdc,bmp2dc
Dim As uint x,y,bmpx,bmpy,itexture
Dim As Any Ptr bmpbits
