...
Dim Shared As Integer autovol=2,reverb=1,noisered=0,bypass=0,mono=0
...
If noisered=0 Then
	noisered=1
	setcheckmenu("win.menu",1)
Else
	noisered=0
	uncheckmenu("win.menu",1)
EndIf
End Sub
Sub submono()
If mono=0 Then
...
Dim Shared As Single mypeek,mypeekin,level,xback0
...
'checkbox("win.noise","noisered",@subnoise,265,158,85,20)
...
openwindow("win","dsp_chung",winx,winy,500,303) 
...
If noisered=1 Then setcheckmenu("win.menu",1)
...
lrestart:
restart=0:quit=0
If auxtest<0.1 Then ttestloop=1
...
Dim Shared As Single xback,xbackold,xbackold0
...
Dim Shared As Single L,C,R,Z,f000,dt,w,t,v(n2,nf),dv(n2,nf),u(n2,nf),du(n2,nf),du2(n2,nf)
...
Sub remove50hz(f00 As Single,i0 As integer)
	...
	Var vv=xback,dvv=(vv-v(ch,fi))/dt
...
Dim As Integer i,j,k
	   ...
	   'f0=440
	   'Var f1=f0*1.031
	   'If Int(Timer/8)And 1 Then f1=f0
	   'xback=20000*Cos(2*pi*f1*timexback) 
		For j=1 To 4
		 For i=1 To 5
			If noisered=1 Then remove50hz(f0*j,i)'*i)
		 Next
		Next 
